...
\cite{fsaeonline_endurance_2026}. Como cada ciclo de projeto concentra meses de
trabalho em poucas oportunidades de validação, decisões de engenharia precisam ser
baseadas em dados confiáveis e obtidos com rapidez.
...
dos pilotos, que é útil, mas insuficiente para decisões de maior precisão.
...
eletrônica embarcada para Fórmula SAE \cite{blatt2020_aquisicao_formula_sae}. A
arquitetura atual utiliza dois módulos de aquisição com microcontroladores STM32,
interligados por um barramento CAN a 1 Mbit/s, que também conecta outros módulos
eletrônicos do veículo \cite{silva2009_rede_can_formula_sae}. Cada módulo lê
...
\begin{itemize}
    ...
    agrupamento de amostras e filtragem por identificador CAN;
    \item desenvolver e comparar duas variantes de transporte, broadcast e
    multicast;
...
A principal contribuição é o WCAN, composto por protocolo e firmware para
...
com configuração de placas, execução por UART, coleta de logs e análise
...
A campanha final, armazenada em \path{transmitter/testing/results/final}, possui 684 execuções
planejadas. Após separar cinco falhas atribuídas à
instrumentação de estatísticas ou log serial, a base útil ficou em 453 aprovações
sobre 679 execuções, ou 66,7\%. O melhor desempenho final foi obtido pelo
multicast com agrupamento temporal. Na suíte \texttt{multiple}, o multicast
aprovou 90 de 90 execuções com múltiplos CAN IDs por sensor, enquanto o broadcast
aprovou 34 de 90. O agrupamento temporal também foi determinante: as suítes com
\texttt{linger\_ms = 100} somaram 415 aprovações em 499 execuções, enquanto a
suíte \texttt{real\_time}, sem agrupamento, somou 38 em 180. Esses resultados
indicam que o WCAN é viável em bancada para aquisição de dados quando usado
dentro dos limites observados, especialmente com multicast e agrupamento de
amostras.
...
firmware e observar gargalos de transporte, mas não reproduz vibração, fixação no
...
entre as variantes broadcast e multicast.

O Capítulo~\ref{chapter:metodos} apresenta a metodologia experimental, incluindo
bancada, builds, suítes de teste, automação, coleta de logs e critérios de
análise.

O Capítulo~\ref{chapter:resultados-testes} discute a campanha final em
\path{transmitter/testing/results/final}, com os resultados por suíte, transporte,
efeito do agrupamento, filtros, falhas representativas e rastreabilidade dos
requisitos.
...
incluindo plano de execução, configurações, resumos e logs usados na análise.
