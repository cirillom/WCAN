...
A principal limitação introduzida pelo ESP-NOW aparece no uso de broadcast. Quando um pacote é enviado para o endereço \texttt{FF:FF:FF:FF:FF:FF}, o transmissor não recebe uma confirmação nativa de entrega associada a cada receptor. Portanto, qualquer garantia de confiabilidade precisa ser construída acima do ESP-NOW, na camada de aplicação. Essa restrição motivou o desenvolvimento de duas estratégias internas do transporte WCAN sobre o mesmo protocolo físico: uma estratégia broadcast, que preserva a semântica de barramento compartilhado do CAN, e uma estratégia multicast, que mantém uma lista de receptores interessados e usa o envio do ESP-NOW para todos os peers cadastrados. As duas estratégias implementam a mesma interface de aplicação; a diferença está na forma de descoberta, confirmação e manutenção dos receptores ativos.

Esse trade-off foi considerado aceitável porque o objetivo do projeto é expandir a aquisição de dados do veículo, não substituir por rádio uma malha de controle crítica. A camada WCAN pode acrescentar acknowledgments, retransmissões, filtragem, descoberta de receptores e medição de perdas, enquanto o ESP-NOW fornece a transmissão imediata e sem infraestrutura que os requisitos exigem. A escolha entre broadcast e multicast, portanto, deve ser guiada pelos resultados de entrega, latência e robustez observados nos ensaios, como também ocorre em outras soluções de telemetria veicular baseadas em Wi-Fi ou ESP32 \cite{fernandes2022_formula_sae_wifi_can}.
