Este apêndice apresenta uma versão sanitizada do arquivo
\path{transmitter/main/main.cpp}\footnote{Disponível em: \url{https://github.com/cirillom/WCAN/blob/main/transmitter/main/main.cpp}.}. O código mantém a estrutura de inicialização do
\emph{firmware}, configuração do Wi-Fi, recebimento da configuração de \emph{boot}
por UART e seleção do papel da placa na campanha experimental. Não há chaves,
endereços privados ou credenciais no trecho apresentado.

\begin{codigo}[language=C++,caption={Arquivo principal do \emph{firmware} WCAN},label={cod:main-cpp}]
#include <cstdio>
#include <memory>
#include <vector>
...
static void init_wifi()
{
    ...
    ESP_ERROR_CHECK(esp_wifi_set_channel(wcan_test::TestConfig::kEspNowChannel,
    ...
    if (primary_channel != wcan_test::TestConfig::kEspNowChannel ||
        second_channel != WIFI_SECOND_CHAN_NONE) {
