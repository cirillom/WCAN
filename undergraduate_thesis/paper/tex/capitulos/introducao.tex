...
A monografia está organizada em quatro capítulos principais. O
Capítulo~\ref{chapter:introducao} apresenta o contexto da Fórmula SAE, o sistema
de aquisição de dados da equipe, as limitações da arquitetura cabeada, os
objetivos, as contribuições e o escopo do trabalho.

O Capítulo~\ref{chapter:desenvolvimento} reúne a construção da solução proposta.
Primeiro são formalizados os requisitos; em seguida, são comparadas as soluções
consideradas e justificada a adoção de uma ponte sem fio. Na sequência, o
capítulo apresenta a justificativa resumida da plataforma, a arquitetura e
implementação do WCAN e a instrumentação usada na metodologia experimental.

O Capítulo~\ref{chapter:resultados-testes} discute a campanha final, com os
resultados por suíte, transporte, efeito do agrupamento temporal, filtros, falhas
representativas e rastreabilidade dos requisitos. O
Capítulo~\ref{chapter:conclusao} sintetiza as conclusões, limitações e trabalhos
futuros, destacando o que foi demonstrado em bancada e o que ainda precisa ser
validado no veículo.

Os apêndices preservam detalhes complementares que não precisam interromper o
texto principal, incluindo a discussão detalhada da escolha da plataforma,
artefatos da campanha experimental, estrutura de automação, \emph{logs} e trecho
do código principal do \emph{firmware}.
