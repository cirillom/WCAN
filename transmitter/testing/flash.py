...
from build import get_build_dir, get_sdkconfig
...
def flash_all(assignments: list, project_path: str = "..") -> bool:
    """
...
def main():
    ...
    if args.port and args.chip and args.role:
        success = flash_board(args.chip, args.role, args.port, args.project_path)
    ...
    if args.board or (not args.port and not args.chip and not args.role):
        ...
        if args.board:
            ...
            success = flash_board(board["chip"], args.role, board["port"], args.project_path)
            sys.exit(0 if success else 1)
        else:
